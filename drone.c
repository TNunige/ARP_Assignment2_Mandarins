...
double repulsive_force(FILE *fp, double coordinate,
                       struct shared_data obstacles[], int dire, double co_y,
                       double co_x) {
  ...
  for (int i = 0; i < NUM_OBSTACLES; i++) {
    ...
    euclidean_distance = sqrt(pow(co_y - obstacles[i].real_y, 2) +
                              pow(co_x - obstacles[i].real_x, 2));
...
int main(int argc, char *argv[]) {
  ...
  if (read(server_drone[0], obstacles, sizeof(obstacles)) == -1) {
    perror("Drone: reading from the server failed");
    exit(EXIT_FAILURE);
  }
  ...
  while (1) {
    ...
    FD_SET(FD[0], &rdfds);  // Put the drone fd in the set
    ...
    // Data is available
      ...
      // If data from the server is available
        ...
        if (type == 1) {
          num_obs = data.num;  // Get the obstacle number to be deleted
          obs_y = data.co_y;   // Get the new obstacle y coordinate (real life)
          obs_x = data.co_x;   // Get the new obstacle x coordinate (real life)

          // Add the new obstacle to the list instead of the old one
          for (int i = 0; i < NUM_OBSTACLES; i++) {
            // Check if the deleted obstacle matches
            if (obstacles[i].real_y == obstacles[num_obs].real_y) {
              // Add the new obstacle coordinates to the list
              obstacles[i].real_y = obs_y;
              obstacles[i].real_x = obs_x;
            }
          }

          // Update the log
          log_message(fp, "New obstacle coordinates real_y", DOUBLE_TYPE,
                      &obs_y);
          log_message(fp, "New obstacle coordinates real_x", DOUBLE_TYPE,
                      &obs_x);
        }
      }
    ...
    // Game continues
      ...
      coordinates[0][y_direction] =
          command_force(fp, coordinates, Fy, y_direction, obstacles);
      // Force in the x direction
      coordinates[0][x_direction] =
          command_force(fp, coordinates, Fx, x_direction, obstacles);
